...
static inline int sysfs_attr_get_value(struct sysfs_device *dev, char *name,
	char *value, int len)
{
	char *path;
	int ret;

	if (asprintf(&path, "%s/%s", dev->path, name) < 0)
		return -1;

	ret = sysfs_read_attribute_value(path, value, len);
	free(path);
	return ret;
}
...
static int scsi_inquiry(struct sysfs_device *dev, int fd,
...
static int do_scsi_page0_inquiry(struct sysfs_device *dev, int fd,
				 unsigned char *buffer, unsigned int len)
{
	...
	memset(buffer, 0, len);
	retval = scsi_inquiry(dev, fd, 1, 0x0, buffer, len);
	if (retval < 0)
		return 1;

	if (buffer[1] != 0) {
		info("%s: page 0 not available.", dev->kernel_name);
		return 1;
	}
	if (buffer[3] > len) {
		info("%s: page 0 buffer too long %d", dev->kernel_name,  buffer[3]);
		return 1;
	}
	...
	if (buffer[3] > MODEL_LENGTH) {
		 ...
		 * invalid.
		 */
		if (sysfs_attr_get_value(dev, "vendor", vendor, 256) < 0) {
			info("%s: cannot get model attribute", dev->kernel_name);
			return 1;
		}
		if (!strncmp((char *)&buffer[VENDOR_LENGTH], vendor, VENDOR_LENGTH)) {
			info("%s: invalid page0 data", dev->kernel_name);
			return 1;
		}
	}
	return 0;
 ...
 * model.
 */
static int prepend_vendor_model(struct sysfs_device *dev, char *serial)
{
	const char *attr;
	int ind;
	char value[1024];

	if (sysfs_attr_get_value(dev, "vendor", value, 1024) < 0) {
		info("%s: cannot get vendor attribute", dev->kernel_name);
		return 1;
	}
	strncpy(serial, attr, VENDOR_LENGTH);
	ind = strlen(serial) - 1;

	if (sysfs_attr_get_value(dev, "model", value, 1024) < 0) {
		info("%s: cannot get model attribute", dev->kernel_name);
		return 1;
	}
	...
	if (ind != (VENDOR_LENGTH + MODEL_LENGTH)) {
		info("%s: expected length %d, got length %d",
		     dev->kernel_name, (VENDOR_LENGTH + MODEL_LENGTH), ind);
		return 1;
	}
	return ind;
}
 ...
 * serial number.
 **/
static int check_fill_0x83_id(struct sysfs_device *dev,
			      ...
			      *id_search, char *serial, int max_len)
{
	 ...
	 */
	if (id_search->id_type == SCSI_ID_VENDOR_SPECIFIC)
		if (prepend_vendor_model(dev, &serial[1]) < 0) {
			dbg("prepend failed\n");
			return 1;
		}
...
static int check_fill_0x83_prespc3(struct sysfs_device *dev,
			           unsigned char *page_83,
			           const struct scsi_id_search_values
			           *id_search, char *serial, int max_len)
{
...
static int do_scsi_page83_inquiry(struct sysfs_device *dev, int fd,
				  char *serial, int len)
{
	int retval;
	...
	retval = scsi_inquiry(dev, fd, 1, PAGE_83, page_83,
			      SCSI_INQ_BUFF_LEN);
	if (retval < 0)
		return 1;

	if (page_83[1] != PAGE_83) {
		info("%s: Invalid page 0x83", dev->kernel_name);
		return 1;
	}
	...
	if (page_83[6] != 0) 
		return check_fill_0x83_prespc3(dev, page_83, 
	...
	for (id_ind = 0;
		...
		for (j = 4; j <= (unsigned int)page_83[3] + 3; j += page_83[j + 3] + 4) {
			retval = check_fill_0x83_id(dev, &page_83[j],
 ...
 */
static int do_scsi_page83_prespc3_inquiry(struct sysfs_device *dev, int fd,
				          char *serial, int len)
{
	int retval;
	int i, j;
	...
	memset(page_83, 0, SCSI_INQ_BUFF_LEN);
	retval = scsi_inquiry(dev, fd, 1, PAGE_83, page_83, SCSI_INQ_BUFF_LEN);
	if (retval < 0)
		return 1;
...
static int do_scsi_page80_inquiry(struct sysfs_device *dev, int fd,
				  char *serial, int max_len)
{
	...
	retval = scsi_inquiry(dev, fd, 1, PAGE_80, buf, SCSI_INQ_BUFF_LEN);
	if (retval < 0)
		return retval;

	if (buf[1] != PAGE_80) {
		info("%s: Invalid page 0x80", dev->kernel_name);
		return 1;
	}
	 ...
	 */
	serial[0] = 'S';
	ser_ind = prepend_vendor_model(dev, &serial[1]);
	if (ser_ind < 0)
		return 1;
	len = buf[3];
...
int scsi_get_serial (struct sysfs_device *dev, const char *devname,
		     int page_code, char *serial, int len)
{
	...
	if (fd < 0) {
		info("%s: cannot open %s: %s",
		    dev->kernel_name, devname, strerror(errno));
		return 1;
	}

	if (page_code == PAGE_80) {
		if (do_scsi_page80_inquiry(dev, fd, serial, len)) {
			retval = 1;
			goto completed;
		} else  {
			retval = 0;
		...
		if (do_scsi_page83_inquiry(dev, fd, serial, len)) {
			retval = 1;
			goto completed;
		} else  {
			retval = 0;
		...
		retval = do_scsi_page83_prespc3_inquiry(dev, fd, serial, len);
		if (retval) {
			/*
		...
		info("%s: unsupported page code 0x%d", dev->kernel_name, page_code);
		return 1;
	}
	 ...
	 */
	if (do_scsi_page0_inquiry(dev, fd, page0, SCSI_INQ_BUFF_LEN)) {
		/*
	...
	for (ind = 4; ind <= page0[3] + 3; ind++)
		if (page0[ind] == PAGE_83)
			if (!do_scsi_page83_inquiry(dev, fd, serial,
						    len)) {
				/*
				 * Success
				 */
	...
	for (ind = 4; ind <= page0[3] + 3; ind++)
		if (page0[ind] == PAGE_80)
			if (!do_scsi_page80_inquiry(dev, fd, serial,
						    len)) {
				/*
				 * Success
				 */
...
completed:
	if (close(fd) < 0)
		info("%s: close failed: %s", dev->kernel_name, strerror(errno));
	return retval;
}

static int scsi_probe(struct sysfs_device *dev, char **id)
{
	char *serial = NULL, *devname = NULL;
	unsigned char page0[SCSI_INQ_BUFF_LEN];
	int ind, fd = -1;
	int ret = -1;

	if (asprintf(&devname, "/dev/%s", dev->name) < 0)
		goto err;
	...
	if (!(serial = calloc(256, sizeof(char))))
		goto err;

	ret = do_scsi_page0_inquiry(dev, fd, page0, SCSI_INQ_BUFF_LEN);
	if (ret == -1)
		goto err;

	for (ind = 4; ind <= page0[3] + 3; ind++) {
		if (page0[ind] == PAGE_83) {
			ret = do_scsi_page83_inquiry(dev, fd, serial, 256);
			if (ret != -1)
				break;
		}
	}
	for (ind = 4; ind <= page0[3] + 3; ind++) {
		if (page0[ind] == PAGE_80) {
			ret = do_scsi_page80_inquiry(dev, fd, serial, 256);
			if (ret != -1)
				break;
		}
	}
